...
#define FREQUENCY 3000000000u // Hz for timing conversion (3.0 GHz)
#define RUNS 10
#define N 10000
#define MAX 1000
#define DEFAULT_THREADS 1
#define ITERATIONS_PER_THREAD 1000000u 
#define TOTAL_ITERATIONS 747483642u // total work
...
{
    char brandString[64];
    char vendorString[16]; // "GenuineIntel", "AuthenticAMD"
...
__int64 BenchmarkMonteCarloBatch(unsigned int numThreads) //Cod preluat si adaptat din laboratorul 1 si 2 - Structura sistemelor de calcul, Universitatea Tehnica din Cluj-Napoca
{
    ...
    // De curiozitate voiam sa vad cat da valoarea
    unsigned long long totalHits = 0;
    unsigned long long totalDone = 0;
    for (unsigned int i = 0; i < numThreads; i++) 
    {
        totalHits += args[i].hits;
        totalDone += args[i].iterations;
    }
    double pi_est = 4.0 * (double)totalHits / (double)totalDone;
...
static void MatrixMulDoWork(ThreadArgsMatrix* arg)
{
    ...
    // guard (debug only)
    if (iStart > num || iEnd > num || iStart > iEnd) return;
    ...
    for (unsigned int i = iStart; i < iEnd; i++) {
        ...
        for (unsigned int j = 0; j < num; j++) {
            ...
            for (unsigned int k = 0; k < num; k++) {
                sum += Ai[k] * B[k * num + j];   // <- crashes here if A/B invalid or iEnd>num
            }
            Ci[j] = sum;
        }
    }
...
__int64 BenchmarkMatrixBatch(unsigned int numThreads)
{
   ...
   /*     printf("MATRIX MUL BENCH \n");
        printf("Size: %ux%u * %ux%u -> %ux%u\n", MM_NR, MM_NR, MM_NR, MM_NR, MM_NR, MM_NR);
        printf("Threads           : %u\n", numThreads);
        printf("Total cycles      : %lld\n", total_cycles);
        printf("Seconds (est)     : %.6f\n", seconds);*/
...
__int64 BenchmarkFFTBatch(unsigned int numThreads)
{
    ...
    {
        printf("FFT alloc failed (N=%u, blocks=%u)\n", nr, BLOCKS);
    ...
    for (unsigned int i = 0; i < numThreads; ++i) {
        ...
        if (!threadHandles[i]) {
            ...
            printf("CreateThread failed for i=%u (GLE=%lu)\n", i, GetLastError());
        }
        cursor += cnt;
    }
    ...
    // Approx operation count (complex radix-2): 5*N*log2(N) per FFT
    double seconds = (double)total_cycles / (double)FREQUENCY;

    //printf(" FFT BENCH \n");
    //printf("FFT length (N)     : %u\n", N);
    //printf("FFT blocks         : %u\n", BLOCKS);
    //printf("Threads            : %u\n", numThreads);
    //printf("Total cycles       : %lld\n", total_cycles);
    //printf("Seconds (est)      : %.6f\n", seconds);
...
void PrintCpuInfo(const CpuInfo* info)
{
    ...
    printf("L1D Cache (KB)  : %d\n", info->l1DataKB);
    printf("L1I Cache (KB)  : %d\n", info->l1InstrKB);
    printf("L2 Cache  (KB)  : %d\n", info->l2KB);
    printf("L3 Cache  (KB)  : %d\n", info->l3KB);
}

void PrintBenchmarkResult_MonteCarlo(unsigned int totalIterations, unsigned int numThreads, const BenchmarkResult* r)
{
...
int main()
{
    ...
    std::cout << "Monte carlon\n";
    std::cout << BenchmarkMonteCarloBatch(1) << "\n";
    std::cout << BenchmarkMonteCarloBatch(2) << "\n";
    std::cout << BenchmarkMonteCarloBatch(4) << "\n";
    std::cout << BenchmarkMonteCarloBatch(8) << "\n";
    std::cout << BenchmarkMonteCarloBatch(16) << "\n";
    std::cout << BenchmarkMonteCarloBatch(32) << "\n";
    std::cout << BenchmarkMonteCarloBatch(64) << "\n";

    std::cout << "Matrix multiply\n";
    std::cout << BenchmarkMatrixBatch(1) << "\n";
    std::cout << BenchmarkMatrixBatch(2) << "\n";
    std::cout << BenchmarkMatrixBatch(4) << "\n";
    std::cout << BenchmarkMatrixBatch(8) << "\n";
    std::cout << BenchmarkMatrixBatch(16) << "\n";
    std::cout << BenchmarkMatrixBatch(32) << "\n";
    std::cout << BenchmarkMatrixBatch(64) << "\n";

    std::cout << "FFT\n";
    std::cout << BenchmarkFFTBatch(1) << "\n";
    std::cout << BenchmarkFFTBatch(2) << "\n";
    std::cout << BenchmarkFFTBatch(4) << "\n";
    std::cout << BenchmarkFFTBatch(8) << "\n";
    std::cout << BenchmarkFFTBatch(16) << "\n";
    std::cout << BenchmarkFFTBatch(32) << "\n";
    std::cout << BenchmarkFFTBatch(64) << "\n";
    return 0;
}
